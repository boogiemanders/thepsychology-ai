...
const VideoPlayer = ({
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {caption && showControls && isPlaying && (
          <motion.div
            key="caption"
            className="pointer-events-none absolute inset-x-0 bottom-32 flex justify-center px-6 sm:bottom-36"
            initial={{ y: 6, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 6, opacity: 0 }}
            transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
          >
            <p className="max-w-[36ch] text-balance text-center text-[13px] font-medium leading-[1.5] tracking-[-0.005em] text-white/95 drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)] sm:text-[15px]">
              {caption}
            </p>
          </motion.div>
        )}
