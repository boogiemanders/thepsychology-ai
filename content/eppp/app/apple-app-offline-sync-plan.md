...
## Multi-App Architecture Decision

### Three Native Apps, One Shared Backend

| App | Audience | Core Purpose | Build Order |
|-----|----------|-------------|-------------|
| **EPPP Study** | Post-docs / students | Offline exam prep, SRS, widgets, adaptive study | **1st** (~21 weeks) |
| **Provider** | Licensed therapists | Dashboard, calendar, clients, outcomes, recordings | **3rd** (after web provider dashboard proves useful) |
| **Find a Therapist** | Clients seeking therapy | Matching, insurance, booking, sessions | **4th** (after matching has real volume) |

The web app at `thepsychology.ai` remains the unified platform with all route groups (`/dashboard`, `/provider/*`, `/find-therapist/*`). Native apps are focused frontends for specific audiences.

### Why Separate Apps

- **App Store requires focused purpose.** Apple rejects apps that try to be too many things. "Study for your exam" vs "manage your practice" vs "find a therapist" are three distinct App Store categories.
- **Different offline needs.** Study app needs heavy offline (lessons, exams, audio). Provider app needs real-time (calendar, video, alerts). Client app needs insurance verification and booking — mostly online.
- **Different update cycles.** Study content updates weekly; provider features update monthly.
- **Different widgets.** Practice questions and streaks vs next appointment and client alerts.
- **Simpler App Store review.** Each app has a focused scope and fewer rejection risks.

### What's Shared Across All Three Apps

- **Supabase backend** — same database, same RLS policies, same auth
- **API routes** — the Next.js web app serves all audiences via route groups
- **User accounts** — same Supabase auth, role-based access (`student | client | provider | admin`)
- **The EPPP pipeline** — when a student passes EPPP, their account gains the `provider` role and the Provider app becomes available

### Build Sequence

1. **EPPP Study app** (this plan) — existing audience, proven product-market fit, ~21 weeks
2. **Web matching/booking** (see `plan-matching-insurance-booking.md`) — web-first, Phases 1-5, no native app needed yet
3. **Provider iOS app** — only after web provider dashboard has active users
4. **Client iOS app** — only after matching platform has real volume in CA + NY

### Shared iOS Infrastructure

To avoid rebuilding common patterns three times:

- **Shared Swift package**: `ThePsychologyCore` — Supabase auth, sync engine, entitlement check, push notifications, App Group storage
- All three apps import this package
- Each app has its own Xcode project, bundle ID, App Store listing, and widget extensions
- Shared package versioned independently so apps can update at their own pace
...
See the full monetization decision doc:

- [`content/strategy/apple-app-monetization.md`](/Users/anderschan/thepsychology-ai/content/strategy/apple-app-monetization.md)

Summary:

- Existing web subscribers sign in and get full access (no StoreKit needed)
- New users who discover the app first can subscribe via StoreKit In-App Purchase
- Supabase `subscriptions` table is the single source of truth
- Both Stripe webhooks and App Store Server Notifications V2 write to it
- App caches entitlement locally for offline use with a 7-day grace period

---

## API Authentication for Mobile

The plan's sync endpoints need authenticated access. Here is how:

### Auth Strategy

- Mobile app uses **Supabase Auth** (same as web)
- User signs in with email/password or magic link
- App receives a JWT access token + refresh token
- All API calls include the JWT in the `Authorization: Bearer` header
- Supabase RLS policies already enforce row-level access -- no new auth layer needed

### Token Lifecycle

- Tokens refresh on app launch and before each sync
- Supabase refresh tokens are long-lived -- works across offline gaps
- If refresh token expires (very long offline), user re-authenticates on next online session
...
| device_name | text | "Anders's iPhone" |
| push_token | text | APNs token for notifications |
| last_sync_at | timestamptz | Last successful sync |
| app_version | text | For compatibility checks |
...
When lessons or questions are updated on the web while a user has old versions downloaded:

### How It Works

- Every bundle in the manifest includes `version` (semver) and `checksum` (SHA-256)
- On each sync, app compares local manifest against server manifest
- Stale bundles are **flagged**, not auto-deleted
- Background download queues updated bundles when on Wi-Fi
- **Never delete an in-progress exam's content mid-session**

### Update Flow

1. App fetches latest `mobile-content-manifest.json`
2. Compares each bundle's checksum against local copy
3. Mismatches get queued for background download
4. User sees a subtle "Updates available" badge on affected lessons/exams
5. Updated content replaces old content only after download completes
6. In-progress exams continue with the version they started on

### Breaking Changes

- If a question is removed or an answer key changes, the bundle version bumps major
- App handles major version bumps by marking the old exam as "outdated" and prompting re-download
- Old exam results remain valid -- they were correct at the time
...
- 82 lessons x ~5-10MB audio each = **~400-800MB total**
- Individual lesson audio: 3-12MB depending on length
- Full exam audio (if any): negligible (exams are text-based)

### Storage Strategy

- **Default**: Audio downloads are opt-in, not bundled with lessons
- **"Light" pack**: Questions + explanations + lesson text only (~50KB per lesson)
- **"Full" pack**: Everything including audio (~8MB per lesson)
- **"Travel" pack**: Current exam + next exam + weak-area lessons with audio

### Storage Management

- Show total storage used in app settings
- Per-pack size breakdown before download
- Auto-evict completed lesson audio after 30 days (user-configurable)
- Manual "clear all downloads" option
- Warn at 1GB, hard prompt at 2GB

---

## Revised Phase Plan (Solo Development)

The original 12-week timeline assumed an experienced iOS developer. For self-taught SwiftUI development, the revised timeline is **~21 weeks**.

### Phase 0: Product and Data Contract (2 weeks)

- Lock down bundle types and sync event schema
- Finalize monetization decision (hybrid -- done)
- Define local storage budget
- Set up Apple Developer account and Xcode project
- Complete Apple's SwiftUI tutorials

### Phase 1: Backend Packaging + Sync Foundation (3 weeks)

- Export lesson, exam, audio, and recommendation manifests
- Add versioned bundle endpoints
- Add bootstrap and sync endpoints
- Create `lesson_progress` and `mobile_devices` tables
- Extract exam persistence into shared server service
- Add entitlement check endpoint
- **Learning**: Supabase Swift SDK, REST API patterns

**Deliverable**: Mobile app can sign in, fetch manifest, and sync a test event

### Phase 2: iPhone App Shell + Offline Library (5 weeks)

- SwiftUI app shell with tab navigation
- Supabase auth integration
- Local SQLite database (App Group container)
- Background download manager (URLSession)
- Offline lesson reader (markdown rendering)
- Offline audio playback
- Offline exam runner (study + test mode)
- **Domain mastery heatmap** on dashboard
- **Learning**: SwiftUI layouts, navigation, URLSession background downloads

**Deliverable**: Download a lesson or exam, airplane mode, keep studying

### Phase 3: Adaptive Study Path + SRS (3 weeks)

- Port priority engine using exported config JSON
- Compute weak areas on-device after exam
- Auto-queue recommended lesson downloads
- Track lesson completion
- Prefetch next practice exam at completion threshold
- **Spaced repetition engine**: schedule missed questions at 1/3/7/21-day intervals
- SRS queue feeds into Quick Question widget data

**Deliverable**: Full "exam -> lessons -> next exam" loop works end-to-end

### Phase 4: Widgets + Notifications (3 weeks)

- Quick Question widget (reads from SRS queue)
- Continue Studying widget
- Exam Countdown widget
- **Study streak tracking** and daily notification
- Reminders for downloaded-but-unfinished packs
- Siri Shortcuts / App Intents
- **Learning**: WidgetKit, App Intents, UNNotifications

**Deliverable**: App is useful in 30-second micro-sessions

### Phase 5: StoreKit + Entitlements (2 weeks)

- StoreKit 2 subscription flow
- Restore purchases
- Entitlement sync (StoreKit <-> Supabase <-> Stripe)
- App Store Server Notifications V2 webhook
- Free tier content gating
- **Learning**: StoreKit 2 APIs, sandbox testing

**Deliverable**: Full purchase flow works in sandbox

### Phase 6: TestFlight Hardening (3 weeks)

- Full offline + reconnect flow testing
- Partial download and resume testing
- Cross-device sync (phone + web) testing
- Widget freshness and App Group data sharing testing
- StoreKit sandbox edge cases
- Beta with real post-docs
- Prepare App Store submission materials

**Deliverable**: Stable TestFlight candidate
...
### Spaced Repetition on Missed Questions

After a question is answered incorrectly, it enters the SRS queue:

- **Interval schedule**: 1 day -> 3 days -> 7 days -> 21 days -> 60 days
- Correct answer: advance to next interval
- Wrong answer: reset to 1 day
- Schedule stored in local SQLite, syncs to server
- SRS queue is the primary source for the Quick Question widget
- When SRS queue is empty, widget falls back to weak-area questions

This is the **#1 differentiator** vs competitors who just show a static "review missed" list. Spaced repetition is proven to improve long-term retention for exam prep.

### Domain Mastery Heatmap

A single visual showing all 8 EPPP domains:

- Color-coded: red (< 50%), yellow (50-70%), green (> 70%)
- Shows mastery percentage and question count per domain
- Shareable as screenshot (post-docs send to supervisors and study partners)
- Data sourced from priority calculator and exam results
- Updates after every exam and quiz completion

### Study Streak Notifications

- Track daily study activity (any question answered OR lesson progress made)
- Single daily notification at the user's usual study time
- Adaptive timing: learns when the user typically studies
- Message examples:
  - "Your 12-day streak is alive -- 5 questions keeps it going"
  - "You missed yesterday. Start fresh today with a quick 5-question sprint"
- Not gamified -- no badges or leaderboards. Just gentle accountability.

---

## V1.5 Features (Post-Launch)

### Exam Anxiety Toolkit

- **Pre-exam**: 2-minute guided breathing exercise with visual pacer
- **Pre-exam**: Confidence card -- "You've completed X hours of prep, Y practice exams, and Z lessons"
- **Post-exam**: Emotional check-in ("How are you feeling?") -- acknowledged, not scored
- **Post-exam low score**: Route to Recovery Mode (confidence rebuild set, short wins playlist)
- This audience is under real cognitive and emotional load. Showing you understand that builds deep loyalty.

### "Teach It Back" Mode

- After completing a lesson, prompt: "Explain [concept] in your own words"
- Voice recording (preferred) or text input
- Not graded or AI-analyzed in v1.5 -- the act of retrieval + elaboration is the value
- Research shows retrieval practice + elaboration beats passive review for long-term retention
- Stored locally, synced as study session metadata
- Future: AI feedback on explanations, gap detection
