#!/usr/bin/env node
/**
 * Generate 20 questions for 52 source files with proper rate limiting
 */
...
// Define all 50 files (based on actually available files)
...
async function generateQuestionsWithClaude(content, knId, fileName) {
  ...
  const prompt = `Generate exactly 20 multiple-choice questions from this material. Return ONLY a JSON array of 20 objects with fields: id, difficulty, type, question, options (A-D), correct_answer, kn_id, explanation.

Material: ${content.substring(0, 3000)}

Distribution: 50% easy/basic, 20% medium/distinction, 30% hard/difficult.
Keep answers balanced (max 15 char variance per question).
KN ID: ${knId}`;
        ...
        try {
          ...
          if (response.content && response.content[0]) {
            ...
            if (Array.isArray(questions) && questions.length === 20) {
              ...
              console.log(`  Got ${questions.length || 0} questions instead of 20`);
...
async function processFiles() {
  ...
  for (let i = 0; i < PRIORITY_FILES.length; i++) {
    ...
    console.log(`\n[${i + 1}/${PRIORITY_FILES.length}] ${fileConfig.source}`);

    const content = readSourceFile(sourceFile);
    if (!content) {
    ...
    if (!questions || questions.length !== 20) {
      console.log(`  ERROR: Only got ${questions.length || 0} questions`);
      failed++;
      failedFiles.push(fileConfig.source);
      continue;
    }
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`SUMMARY: ${successful}/${PRIORITY_FILES.length} successful`);
  if (failed > 0) {
