...
- Fires at 7:00 AM local, with catch-up when Chrome was closed (Chrome is "mostly open").
...
1. `chrome.alarms` fires (`rater8-daily`, next 7:00 AM local, `periodInMinutes: 1440`). `onStartup`/`onInstalled` re-arm the alarm and run a missed-day check: if `lastUploadedThrough` < yesterday and no run is in flight, run now (this is catch-up).
...
8. On failure: Slack failure message naming the broken step, red badge (`chrome.action.setBadgeText`), surface the tab that needs login (mirrors existing SP behavior), mark nothing as sent. Recovery = founder fixes login and clicks "Upload to rater8 now", or the next alarm/startup catch-up retries.
...
- Failure: `rater8 upload FAILED at <SP pull | rater8 upload>: <log into SimplePractice | log into rater8 | HTTP 500>. Open the extension and click "Upload to rater8 now" after fixing it.`
